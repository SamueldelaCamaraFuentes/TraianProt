...
identify_proteins <- function(raw, metadata, platform, selected_conditions) {
  ...
  # This is the key part. We return the plot object we just created.
  return(plot_object)
}
#Venn Diagram
...
filter_valids <- function(df, metadata, unique_proteins, min_prop = NULL,
  ...
  row_cv_intensity <- function(mat) {
    # 1. Exponenciar: Convertir de log2(Intensidad) a Intensidad
    intensity_mat <- 2^mat
    row_sds <- apply(intensity_mat, 1, sd, na.rm = TRUE)
    row_means <- rowMeans(intensity_mat, na.rm = TRUE) 
    cv <- (row_sds / row_means)
    cv[is.nan(cv)] <- NA 
    
    return(cv) # CV en porcentaje (%)
  }
  
  common_df$CV_Control <- row_cv_intensity(valuesA)
  common_df$CV_Tratamiento <- row_cv_intensity(valuesB)
  common_df$CV_TratamientoyControl <- row_cv_intensity(valuesC)
  # ---------------------------------------------------------------------------
...
boxplot_function <- function(df, metadata, selected_conditions) {
  ...
  plot_data_long$Sample <- factor(plot_data_long$Sample, levels = cond.names)
...
corrplot_function <- function(df, display = "circle", tl.col = "black", addCoef.col = "black") {
  ...
  raw_var_names <- colnames(df_cor)
  
  var_names_ordered <- gtools::mixedsort(raw_var_names) 

  melted_cormat <- as.data.frame(df_cor) %>%
    tibble::rownames_to_column("Var1") %>%
    tidyr::pivot_longer(-Var1, names_to = "Var2", values_to = "value")
  
  melted_cormat$Var1 <- factor(melted_cormat$Var1, levels = var_names_ordered)
  melted_cormat$Var2 <- factor(melted_cormat$Var2, levels = var_names_ordered)
...
tsne <- function(x, metadata, perplexity_num, selected_conditions){
  ...
  x <- na.omit(x[group]) # na.omit is base R
  
  tdata<-as.data.frame(t(x[group])) # as.data.frame and t are base R
  
  set.seed(5) #For reproducibility
  tsne <- Rtsne::Rtsne(X = tdata, perplexity = as.numeric(perplexity_num))
  ...
  ggplot2::ggplot(tsne_df, ggplot2::aes(x = Dim1, y = Dim2, label = Sample, color = Group)) +
    ggplot2::geom_point(size = 4) +
    ggplot2::geom_text(vjust = -0.5, hjust = 0.5, size = 3) +
    ggplot2::stat_ellipse(ggplot2::aes(group = Group), linetype = "dashed", size = 1) +
    ggplot2::labs(title = "t-SNE Plot", x = "t-SNE 1", y = "t-SNE 2") +
    ggplot2::theme_minimal()
