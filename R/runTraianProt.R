...
runTraianProt <- function() {
                       ...
                       sidebarMenu(
                         menuItem("File Input", tabName = "file input", icon = icon("upload"),
                                  ...
                                  fileInput(inputId = "metadata",
                                            ...
                                            accept = c("text/csv",
                                                       ".csv")),
                                  switchInput(inputId = "technicalreplicates",
                         ...
                         menuItem("Venn Diagram", icon = icon("th"), tabName = "venn",
                                  ...
                                  downloadButton(outputId = "downloadvenn",
                                                 ...
                                                 icon = icon("download"),
                                                 style="display: block; margin: 0 auto; width: 200px; color:black;")),
                         menuItem("Power of a test", icon = icon("th"), tabName = "powertest",
                                  strong(h4("Fold Change")),
                                  numericInput(inputId = "FoldChangepower",
                                               label = "Fold change",
                                               value = 2),
                                  numericInput(inputId = "replicatespower",
                                               label = "Maximun number of replicates",
                                               value = 30),
                                  selectInput(inputId = "powerchoice",
                                              label = "Choice:",
                                              choices = list("p value" = 1,
                                                             "adjusted p value" = 2,
                                                             selected = 2)),
                                  numericInput(inputId = "statisticalpower",
                                               label = "value",
                                               value = 0.05)
                         ),
                         ...
                         menuItem( "Quality metrics", icon = icon("chart-line"),
                                   ...
                                   selectInput(inputId = "dimreduction",
                                               ...
                                               choices = list("PCA" = 1,
                                                              "t-SNE" = 2),
                                               selected = 1),
                                   conditionalPanel(condition = "input.dimreduction == '1'", # Only show for PCA
                                                    selectInput(inputId = "pc_x_axis",
                                                                label = "Select PC for X-axis:",
                                                                choices = 1:5, # Start with a reasonable range, e.g., PC1 to PC5
                                                                selected = 1),
                                                    selectInput(inputId = "pc_y_axis",
                  ...
                  tabPanel(title = "Preprocessing",
                           ...
                           fluidPage(
                             fluidRow(
                               ...
                               box(
                                 title = "Power of a test", width = 6, status = "primary",
                                 plotOutput(outputId = "powertest", height = "600px")
                               ),
                               box(
                                 title = "Note", width = 12, status = "warning", solidHeader = TRUE,
                                 helpText("If your data originates from FragPipe or Proteome Discoverer,
                                        we recommend using the Unique Peptides Extractor tool
                                       (available in the sidebar) before proceeding with the analysis
                                       to ensure proper filtering of unique peptides.")
                               )
  ...
  server <- function(input, output, session) {
    ...
    output$home_img <- renderImage({
      ...
      img_path <- system.file("extdata", "traianprot.png", package = "TraianProt")
    ...
    output$download_tutorial <- downloadHandler(
      ...
      content = function(file) {
        tutorial_path <- system.file("extdata", "Tutorial.pdf", package = "TraianProt")
        file.copy(tutorial_path, file)
      }
    )
    ...
    output$dl_example_matrix <- downloadHandler(
      ...
      content = function(file) {
        tutorial_path <- system.file("extdata", "proteinGroups.txt", package = "TraianProt")
        file.copy(tutorial_path, file)
      }
    )
    ...
    output$dl_example_metadata <- downloadHandler(
      ...
      content = function(file) {
        tutorial_path <- system.file("extdata", "metadata_MaxQuant.tsv", package = "TraianProt")
        file.copy(tutorial_path, file)
      }
    )
    ...
    data_quick_base <- reactive({
      ...
      if (input$comptplatform == 1 | input$comptplatform == 2 | input$comptplatform == 5){
        ...
        df<- quick_filtering(raw, input$comptplatform, input$organismfocus, filtered_metadata_base(), selected_conditions())
        return(df)
      } else if (input$comptplatform == 3){
        req(input$file) #Controls whether a file has been uploaded or not

        raw <- read.delim(input$file$datapath, sep = "\t", stringsAsFactors = FALSE, colClasses = "character", check.names = FALSE)
        df<- quick_filtering(raw, input$comptplatform, input$organismfocus, filtered_metadata_base(), selected_conditions(), selected_dir()) #Marcarlo como dataframe
        df <- as.data.frame(df)
        return(df)
      } else if (input$comptplatform == 4){
    ...
    data_quick <- reactive({
      if (input$technicalreplicates == TRUE) {
        return(collapsed_bundle()$data)
      } else {
        return(data_quick_base())
      }
    })

    filtered_metadata <- reactive({
      if (input$technicalreplicates == TRUE) {
        return(collapsed_bundle()$metadata)
      } else {
        return(filtered_metadata_base())
      }
    })
    ...
    output$protident <- renderPlot({
      try(print(proteins_identified(), silent = TRUE))
    })
    ...
    output$powertest <- renderPlot({
      try(print(power_curve(), silent = TRUE))
    })
    ...
    venn_p <- reactive({venn_diagram(data_filtered(), unique_proteins_reactive(), input$color1, input$color2)})
    ...
    output$boxplot <- renderPlot({
      if (input$displaydistplots == 1) {
        boxplot <- try(print(boxplot_distribution()), silent = TRUE)
    ...
    pca_reactive <- reactive({
      pca(data(),filtered_metadata(), selected_conditions(), pc_x = input$pc_x_axis, pc_y = input$pc_y_axis)
    })

    output$pcaplot <- renderPlot({
      if (input$dimreduction == 1) {
        try(print(pca(data(),filtered_metadata(), selected_conditions(), pc_x = input$pc_x_axis, pc_y = input$pc_y_axis)))
    ...
    output$downloadqmplot <- downloadHandler(
      ...
      content = function(file) {
        if (input$qualitymetricsplotchoice == 1) {
          grDevices::tiff(file, width = 12, height = 10, units = "in", res = 400)
          print(boxplot_function(data(), filtered_metadata(), selected_conditions()))
          grDevices::dev.off()
        } else if (input$qualitymetricsplotchoice == 2) {
          grDevices::tiff(file, width = 5, height = 4, units = "in", res = 300)
          ...
          grDevices::tiff(file, width = 12, height = 10, units = "in", res = 400)
          print(pca(data(),filtered_metadata(), selected_conditions(), pc_x = input$pc_x_axis, pc_y = input$pc_y_axis))
          grDevices::dev.off()
    ...
    output$downloadvolcano <- downloadHandler(
      ...
      content = function(file) {
        if (input$difextension == "tiff"){
          ...
          grDevices::pdf(file, width = 12, height = 10, units = "in", res = 400)
          p <- volcano_plot_tiff(difexpression(), input$volcanotitle, input$labelprots, input$statselected, input$PSMaware)
          print(p)
          grDevices::dev.off()
    ...
    output$downloadreport <- downloadHandler(
      filename = function() {
        paste("Downstreaming_Analysis_report-", Sys.Date(), ".html", sep = "") # <- Pide un HTML
      },
      content = function(file) {
        tempReport <- file.path(tempdir(), "Downstreaming_Analysis_report.Rmd")
        report_template <- system.file("extdata", "Downstreaming_Analysis_report.Rmd", package = "TraianProt")
